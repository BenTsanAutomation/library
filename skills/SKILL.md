---
name: library
description: Official skill for how to use library (the bookmark manager) and interact with it programmatically.
metadata:
  ...
  openclaw:
    envVars:
      - name: LIBRARY_API_KEY
        required: true
        description: The API key for your Library instance
      - name: LIBRARY_SERVER_ADDR
        required: false
        description: The server address for your Library instance.
    requires:
      env:
        - LIBRARY_API_KEY
        - LIBRARY_SERVER_ADDR
    ...
    links:
      repository: https://github.com/your-org/library
      documentation: https://docs.library.example.com
    emoji: 📦
    cliHelp: library --help
    install:
      - kind: node
        package: "@library/cli"
        bins: [library]
...
Library is an open source self-hosted bookmark manager for collecting, organizing, and searching content. This skill covers the core concepts and how to interact with Library via the CLI.

## When to use

Use this skill when the user wants to interact with their Library instance (adding bookmarks, managing lists/tags, searching, etc.).

## Core Concepts

### Bookmarks

- **Bookmarks**: Core entity in Library. Can be one of links, text or media.
  - **Links**: Save URLs — Library auto-fetches title, description, image, screenshot, and full-page archive.
...
Library has a powerful search query language for finding the right bookmarks. It supports full-text search, boolean logic, qualifiers, and more.
...
Library can also be used to consume RSS feeds, but also can itself act as an RSS feed publisher.
...
## Interacting with Library via the CLI
...
The CLI requires an API key and server address. Get the API key from your Library instance's settings page.
...
export LIBRARY_SERVER_ADDR="https://cloud.library.com"
```

**Option 2 — CLI flags:**

```bash
library --api-key <key> --server-addr <addr> <command>
```
...
Run `library --help` to see all available commands, but the most important ones are:

```bash
# Add a link bookmark
library bookmarks add --link "https://example.com"

# Add a link with tags and to a specific list
library bookmarks add --link "https://example.com" --tag-name "reading" --list-id <list-id>

# Add a text bookmark
library bookmarks add --note "Remember to review the PR"

# Get bookmark details
library bookmarks get <bookmark-id>
library bookmarks get <bookmark-id> --include-content

# Update a bookmark
library bookmarks update <bookmark-id> --title "New Title"
library bookmarks update <bookmark-id> --archive
library bookmarks update <bookmark-id> --favourite
library bookmarks update-tags <bookmark-id> --add-tag "important"
library bookmarks update-tags <bookmark-id> --remove-tag "old-tag"

# List management
library lists list
library lists get --list <list-id>
library lists add-bookmark --list <list-id> --bookmark <bookmark-id>
library lists remove-bookmark --list <list-id> --bookmark <bookmark-id>
library lists delete <list-id>
...
# Search bookmarks
library bookmarks search "is:fav #work"
library bookmarks search "rust" --limit 10 --sort-order relevance
library bookmarks search "is:tagged" --all   # paginate through all results

# Delete a bookmark
library bookmarks delete <bookmark-id>
```
