...
import { saveRedditPost } from "./extractors/reddit";
import { snipAndSaveScreenshot } from "./extractors/screenshot";
import { saveTweet } from "./extractors/twitter";
import { saveYouTubeVideo } from "./extractors/youtube";
import { NEW_BOOKMARK_REQUEST_KEY_NAME } from "./protocol";

const OPEN_LIBRARY_ID = "open-library";
const ADD_LINK_TO_LIBRARY_ID = "add-link";
const CLEAR_CURRENT_CACHE_ID = "clear-current-cache";
const CLEAR_ALL_CACHE_ID = "clear-all-cache";
const SEPARATOR_ID = "separator-1";
const VIEW_PAGE_IN_LIBRARY = "view-page-in-library";

// Per-platform context-menu IDs. Additive — these live alongside the existing
// "Add to Library" handler, they do not replace it.
const SAVE_REDDIT_POST_ID = "save-reddit-post";
const SAVE_X_TWEET_ID = "save-x-tweet";
const SAVE_YOUTUBE_VIDEO_ID = "save-youtube-video";
const SNIP_SCREENSHOT_ID = "snip-screenshot";

const REDDIT_POST_PATTERNS = ["*://*.reddit.com/r/*/comments/*"];
const X_TWEET_PATTERNS = [
  "*://x.com/*/status/*",
  "*://twitter.com/*/status/*",
];
 ...
 * * A context menu button to open a tab with the currently configured library instance.
 * * * If the "show count badge" setting is enabled, add context menu buttons to clear the cache for the current page or all pages.
 * * A context menu button to add a link to library without loading the page.
 * @param settings The current plugin settings.
 */
function registerContextMenus(settings: Settings) {
  ...
  chrome.contextMenus.create({
    id: ADD_LINK_TO_LIBRARY_ID,
    title: "Add to Library",
    contexts: ["link", "page", "selection", "image"],
  });

  // Per-platform rich-save menu items. `documentUrlPatterns` restricts the
  // menu to pages on the platform domain, which covers the dominant case
  // (user is browsing the post / tweet / video and saves it).
  chrome.contextMenus.create({
    id: SAVE_REDDIT_POST_ID,
    title: "Save Reddit post (with comments)",
    contexts: ["page", "link"],
    documentUrlPatterns: REDDIT_POST_PATTERNS,
  });

  chrome.contextMenus.create({
    id: SAVE_X_TWEET_ID,
    title: "Save X tweet (single tweet only)",
    contexts: ["page", "link"],
    documentUrlPatterns: X_TWEET_PATTERNS,
  });

  chrome.contextMenus.create({
    id: SAVE_YOUTUBE_VIDEO_ID,
    title: "Save YouTube video (with transcript)",
    contexts: ["page", "link"],
    documentUrlPatterns: YOUTUBE_VIDEO_PATTERNS,
  });

  // Snip & save screenshot — works on any page (no documentUrlPatterns).
  chrome.contextMenus.create({
    id: SNIP_SCREENSHOT_ID,
    title: "Snip & save screenshot",
    contexts: ["page", "action"],
  });

  if (settings?.showCountBadge) {
    chrome.contextMenus.create({
      id: VIEW_PAGE_IN_LIBRARY,
      title: "View this page in Library",
...
/**
 * Handle context menu clicks by opening a new tab with library or adding a link to library.
...
async function handleContextMenuClick(
  ...
  if (menuItemId === OPEN_LIBRARY_ID) {
    ...
    await clearAllCache();
  } else if (menuItemId === ADD_LINK_TO_LIBRARY_ID) {
  ...
  } else if (menuItemId === VIEW_PAGE_IN_LIBRARY) {
    if (tab) {
      await searchCurrentUrl(tab.url);
    }
  } else if (menuItemId === SAVE_REDDIT_POST_ID) {
    if (pageUrl) await saveRedditPost(pageUrl);
  } else if (menuItemId === SAVE_X_TWEET_ID) {
    if (pageUrl) await saveTweet(pageUrl);
  } else if (menuItemId === SAVE_YOUTUBE_VIDEO_ID) {
    if (pageUrl) await saveYouTubeVideo(pageUrl);
  } else if (menuItemId === SNIP_SCREENSHOT_ID) {
    if (tab) await snipAndSaveScreenshot(tab);
  }
}

/**
 * Add a link to library based on the provided information.
...
/**
 * Handle command events, such as adding a link to library.
 * @param command The command to handle.
 * @param tab The current tab.
 */
function handleCommand(command: string, tab: chrome.tabs.Tab) {
  if (command === ADD_LINK_TO_LIBRARY_ID) {
    addLinkToLibrary({
