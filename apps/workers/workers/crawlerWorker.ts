...
import type { ZCrawlLinkRequest } from "@library/shared-server";
...
const tracer = getTracer("@library/workers");
        ...
        if (serverConfig.crawler.enableAdblocker) {
          ...
          const globalBlockerResult = await tryCatch(
            PlaywrightBlocker.fromPrebuiltFull(fetchWithProxy, {
              path: path.join(os.tmpdir(), "library_adblocker.bin"),
              read: fs.readFile,
              write: fs.writeFile,
            }),
          );
