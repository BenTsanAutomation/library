...
if ! port_in_use 7700; then
    ...
    docker run -d -p 7700:7700 --name library-meilisearch getmeili/meilisearch:v1.41.0
else
...
if ! port_in_use 9222; then
    ...
    docker run -d -p 9222:9222 --name library-chrome gcr.io/zenika-hub/alpine-chrome:124 \
        --no-sandbox \
        --disable-gpu \
...
cleanup() {
    ...
    docker stop library-meilisearch library-chrome 2>/dev/null
    docker rm library-meilisearch library-chrome 2>/dev/null
    exit 0
}
