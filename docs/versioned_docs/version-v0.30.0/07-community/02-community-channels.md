# Community Channels

Stay connected with the Library team and community for updates, support, and feature discussions.

## Discord

- Join the official server: [discord.gg/NrgeYywsFh](https://discord.gg/NrgeYywsFh)
- Great for getting help, sharing setups, and chatting with the team and other users.

## Twitter / X

- Follow [@library_app](https://twitter.com/library_app) for release announcements, tips, and product news.
- DM or tag us with feedback or things you'd like to see next.
