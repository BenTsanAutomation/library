...
import type { ZBookmarkTypeLink } from "@library/shared/types/bookmarks";
import {
  getBookmarkLinkImageUrl,
  getBookmarkTitle,
  getSourceUrl,
  isBookmarkStillCrawling,
} from "@library/shared/utils/bookmarkUtils";
...
function LinkImage({
  ...
  if (isBookmarkStillCrawling(bookmark)) {
    body = imgComponent("/blur.avif", false);
  } else if (imageDetails) {
    body = imgComponent(imageDetails.url, !imageDetails.localAsset);
  } else if (title) {
    // No image, but we have a title — render a "title card" placeholder
    // instead of a blank white area. Common for text-only social/forum posts
    // (Reddit selftext, Bluesky, X tweets without media, Hacker News, etc.).
    let host = "";
    try {
    ...
    body = (
      <div className="library-link-titlecard">
        <p className="library-link-titlecard-title">{title}</p>
        {host && (
          <p className="library-link-titlecard-host">{host}</p>
        )}
      </div>
    );
  } else {
    // No image, no title — fall back to the original white pixel so layout
    // still has a defined image slot.
    body = imgComponent(
  ...
  return (
    <Link
      ...
      className={className}
    >
      <div className="relative size-full flex-1">{body}</div>
    </Link>
  );
}
