...
- Also classify the image into one of these contentType values: "article", "product", "book", "recipe", "video", "note", "tweet". Pick the single best match. Use "note" if uncertain.
${curatedInstruction}
${tagStyleInstruction}
${customPrompts && customPrompts.map((p) => `- ${p}`).join("\n")}
You must respond in valid JSON with the keys "tags" (array of strings) and "contentType" (one of the allowed values). Don't wrap the response in a markdown code.`;
}
...
- Also classify the content into one of these contentType values: "article", "product", "book", "recipe", "video", "note", "tweet". Pick the single best match. Use "note" for short user-written notes, "article" for blog posts and news, "product" for shop/landing pages, "tweet" for X/Twitter posts. Default to "note" if uncertain.
${curatedInstruction}
${tagStyleInstruction}
${customPrompts && customPrompts.map((p) => `- ${p}`).join("\n")}
...
You must respond in JSON with the keys "tags" (array of strings) and "contentType" (one of the allowed values).`;
}
