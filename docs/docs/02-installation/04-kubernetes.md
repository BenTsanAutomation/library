...
Using `LIBRARY_VERSION=release` will pull the latest stable version. You might want to pin the version instead to control the upgrades (e.g. `LIBRARY_VERSION=0.10.0`). Check the latest versions [here](https://github.com/your-org/library/pkgs/container/library).
...
` kubectl -n library patch service web -p '{"spec":{"type":"ClusterIP"}}' `

Afterwards you can apply the ingress and access the service via your chosen URL.

#### Setting up HTTPS access to the Service

To access library securely you can configure the ingress to use a preconfigured TLS certificate. This requires that you already have the needed files, namely your .crt and .key file, on hand.

After you have deployed the library manifests you can deploy your certificate for library in the `library` namespace with this example command. You can name the secret however you want. But be aware that the secret name in the ingress definition has to match the secret name.

` $ kubectl --namespace library create secret tls library-web-tls --cert=/path/to/crt --key=/path/to/key `
...
Go to the [quick sharing page](../04-using-library/quick-sharing.md) to install the mobile apps and the browser extensions. Those will help you hoard things faster!

## Updating

Edit the `LIBRARY_VERSION` variable in the `kustomization.yaml` file and run `make clean deploy`.
