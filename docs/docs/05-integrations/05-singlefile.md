...
5. In the `authorization token` field, paste an API key that you can get from your library settings.
...
Now, go to any page and click the singlefile extension icon. Once it's done with the upload, the bookmark should show up in your library instance. Note that the singlefile extension doesn't show any progress on the upload. Given that archives are typically large, it might take 30+ seconds until the upload is done and starts showing up in Library.
...
Also, you most likely will want to change the default `MAX_ASSET_SIZE_MB` in library to something higher, for example `100`.
