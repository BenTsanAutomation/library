...
import type { ZBookmarkList } from "@library/shared/types/lists";
import { MAX_NUM_BOOKMARKS_PER_PAGE } from "@library/shared/types/bookmarks";
import { ZCursor } from "@library/shared/types/pagination";
import { MAX_NUM_TAGS_PER_PAGE } from "@library/shared/types/tags";
...
export const dumpCmd = new Command()
  ...
  .action(async (opts) => {
    ...
    const workRoot = await fsp.mkdtemp(
      path.join(os.tmpdir(), `library-dump-${ts}-`),
    );
    const outFile = opts.output ?? path.resolve(`library-dump-${ts}.tar.gz`);

    try {
      line("");
      line(`${chalk.bold("Library Dump")}`);
