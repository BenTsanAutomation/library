...
describe("Inference Worker Tests", () => {
  const port = inject("libraryPort");

  if (!port) {
    throw new Error("Missing required environment variables");
  }

  let client: ReturnType<typeof createLibraryClient>;
  ...
  it("auto-tags text bookmarks", async () => {
    ...
    const { data: createdBookmark, error } = await client.POST("/bookmarks", {
      body: {
        ...
        text: "Library should tag this bookmark through the inference worker.",
      },
    });
    ...
    expect(bookmark.tags.map((tag) => tag.name)).toEqual(
      expect.arrayContaining(["ai-generated", "library", "worker-test"]),
    );
  }, 120000);
  ...
  it("auto-tags and summarizes crawled link bookmarks", async () => {
    ...
    expect(bookmark.summary).toBe(
      "This page contains a short Hello World test document used to verify Library's inference worker end-to-end.",
    );
    expect(bookmark.tags.map((tag) => tag.name)).toEqual(
      expect.arrayContaining(["ai-generated", "library", "worker-test"]),
    );
