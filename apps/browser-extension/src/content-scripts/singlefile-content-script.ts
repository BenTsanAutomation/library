...
declare global {
  interface Window {
    __librarySingleFileLoaded__?: boolean;
  }
}

if (window.__librarySingleFileLoaded__) {
  // Already registered in this page context — don't re-register listeners.
  // Using `throw` short-circuits re-injection cleanly.
  throw new Error("library singlefile content script already loaded");
}
window.__librarySingleFileLoaded__ = true;
