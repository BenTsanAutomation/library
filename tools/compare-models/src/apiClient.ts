...
export class LibraryAPIClient {
  private readonly client: ReturnType<typeof createLibraryClient>;

  constructor() {
    this.client = createLibraryClient({
      baseUrl: `${config.LIBRARY_SERVER_ADDR}/api/v1/`,
      headers: {
        "Content-Type": "application/json",
        authorization: `Bearer ${config.LIBRARY_API_KEY}`,
      },
    });
  }
