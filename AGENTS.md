...
This document provides context about the Library project for the different agents.

## Project Overview

Library is a monorepo project managed with Turborepo. It appears to be a web application with a focus on collecting and organizing information, possibly a bookmarking or "read-it-later" service. The project is built with a modern tech stack, including:
...
- **`browser-extension`:** A browser extension, likely for saving content to library.
...
- **`mcp`:** The Model Context Protocol (MCP) server to communicate with Library.
