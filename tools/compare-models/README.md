...
A standalone CLI tool to compare the tagging performance of AI models using your existing Library bookmarks.
...
- Fetches existing bookmarks from your Library instance
...
LIBRARY_SERVER_ADDR=https://your-library-instance.com
...
export LIBRARY_SERVER_ADDR=https://your-library-instance.com
...
1. The tool fetches your latest link bookmarks from Library
...
This tool leverages Library's shared infrastructure:
- **API Client**: Uses `@library/sdk` for type-safe API interactions with proper authentication
- **Inference**: Reuses `@library/shared/inference` for OpenAI client with structured output support
- **Prompts**: Uses `@library/shared/prompts` for consistent tagging prompt generation with token management
- No code duplication - all core functionality is shared with the main Library application
...
- Content is fetched with `includeContent=true` from Library API
- Uses Library SDK (`@library/sdk`) for type-safe API interactions
