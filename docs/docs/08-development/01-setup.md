...
Library uses `node` version 24. To install it, you can use `nvm` [^1]
...
Library also makes use of `corepack`[^2]. If you have `node` installed, then `corepack` should already be
...
devDependencies:
+ @library/prettier-config 0.1.0 <- tooling/prettier
...
pnpm install
pnpm --filter @library/mobile clean:prebuild
```
...
| Preview | Standalone app that doesn't require the expo devserver. Uses its own bundle ID. | No | `pnpm --filter @library/mobile ios:preview` | `pnpm --filter @library/mobile android:preview` |
| Release | Standalone app using the production bundle ID. Closest to a production build. | No | `pnpm --filter @library/mobile ios:release` | `pnpm --filter @library/mobile android:release` |
