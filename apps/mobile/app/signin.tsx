...
export default function Signin() {
  ...
  const onSignUp = async () => {
    const serverAddress = settings.address ?? "https://library.example.com";
    const signupUrl = `${serverAddress}/signup?redirectUrl=${encodeURIComponent("library://signin")}&skipSessionRedirect=1`;

    await WebBrowser.openAuthSessionAsync(signupUrl, "library://signin");
  };
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <View className="flex h-full flex-col justify-center gap-2 px-4">
          <View className="items-center">
            <Text className="mb-2 text-xs font-semibold uppercase tracking-[0.24em] text-muted-foreground">
              Library
            </Text>
            <TailwindResolver
          ...
          <View className="gap-2">
            ...
            <View className="flex-row items-center gap-2">
              <View className="flex-1 rounded-md border border-border bg-card px-3 py-2">
                <Text>{settings.address ?? "https://library.example.com"}</Text>
              </View>
              <Button
                size="icon"
                variant="secondary"
