...
import type { ClientConfig } from "@library/shared/config";
import type { AppRouter } from "@library/trpc/routers/_app";
import {
  TRPC_MAX_URL_LENGTH_INTERNAL,
  TRPCProvider,
} from "@library/shared-react/trpc";
          ...
          <QueryClientProvider client={queryClient}>
            <TRPCProvider trpcClient={trpcClient} queryClient={queryClient}>
              <CustomI18nextProvider lang={userLocalSettings.lang}>
                {/*
                  Apply BOTH `.dark` (for shadcn HSL tokens) AND
                  `data-theme="dark"` (for Library's hex tokens defined in
                  globals.css under `:root[data-theme="dark"]`). Without the
                  second attribute, the Library tokens (--fg-primary,
                  --bg-elevated, --accent, …) silently stay at their light
                  defaults in dark mode, producing dark-on-dark contrast
                  failures across the whole UI.
                */}
                <ThemeProvider
