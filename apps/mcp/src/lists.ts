...
import { libraryClient, mcpServer } from "./shared";
...
mcpServer.tool(
  ...
  async (): Promise<CallToolResult> => {
    const res = await libraryClient.GET("/lists", {
      params: {},
    });
    if (!res.data) {
...
mcpServer.tool(
  ...
  async ({ listId, bookmarkId }): Promise<CallToolResult> => {
    const res = await libraryClient.PUT(
...
mcpServer.tool(
  ...
  async ({ listId, bookmarkId }): Promise<CallToolResult> => {
    const res = await libraryClient.DELETE(
      `/lists/{listId}/bookmarks/{bookmarkId}`,
      {
        params: {
          path: {
...
mcpServer.tool(
  ...
  async ({ name, icon, parentId }): Promise<CallToolResult> => {
    const res = await libraryClient.POST("/lists", {
      body: {
        name,
        icon,
        parentId,
