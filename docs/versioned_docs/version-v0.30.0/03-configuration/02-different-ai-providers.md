...
Library uses LLM providers for AI tagging and summarization. We support OpenAI-compatible providers and ollama. This guide will show you how to configure different providers.
...
Ollama is a local LLM provider that you can use to run your own LLM server. You'll need to pass ollama's address to library and you need to ensure that it's accessible from within the library container (e.g. no localhost addresses).
