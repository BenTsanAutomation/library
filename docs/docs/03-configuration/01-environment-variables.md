...
The app is mainly configured by environment variables. All the used environment variables are listed in [packages/shared/config.ts](https://github.com/your-org/library/blob/main/packages/shared/config.ts). The most important ones are:
...
| LOG_LEVEL                              | No                                    | debug           | The application log level as defined in the [winston documentation](https://github.com/winstonjs/winston?tab=readme-ov-file#logging-levels). You may want to set this to `notice` or `warning` when running Library in a production environment.                      |
...
Library supports two storage backends for assets: local filesystem (default) and S3-compatible object storage. S3 storage is automatically detected when an S3 endpoint is passed.
...
By default, Library uses the database to store users, but it is possible to also use OAuth.
...
:::info
When setting up OAuth, the allowed redirect URLs configured at the provider should be set to `<LIBRARY_ADDRESS>/api/auth/callback/custom` where `<LIBRARY_ADDRESS>` is the address you configured in `NEXTAUTH_URL` (for example: `https://try.library.example.com/api/auth/callback/custom`).
:::
...
| OAUTH_ALLOW_DANGEROUS_EMAIL_ACCOUNT_LINKING | No       | false                  | Whether existing accounts in library stored in the database should automatically be linked with your OAuth account. Only enable it if you trust the OAuth provider!                                  |
...
- You can use the placeholders `$tags`, `$aiTags`, `$userTags` in the prompt. These placeholders will be replaced with all tags, ai generated tags or human created tags when automatic tagging is performed (e.g. `[library, computer, ai]`)
  :::
...
Library can send emails for various purposes such as email verification during signup. Configure these settings to enable email functionality.
...
If your Library instance needs to connect through a proxy server, you can configure the following settings:
...
Library supports distributed tracing and structured event logging via OpenTelemetry. When enabled, traces and logs are collected for tRPC API calls, background worker operations, and other key workflows. Library also exports prometheus-based metrics.
...
| OTEL_SERVICE_NAME                  | No       | library | The service name that will appear in your tracing backend. The actual service name will include a suffix (e.g., `library-api`, `library-workers`).                                                                                                                                                                    |
