...
} from "@library/shared-react/hooks/assets";
import { useBookmarkGridContext } from "@library/shared-react/hooks/bookmark-grid-context";
...
import { useRemoveBookmarkFromList } from "@library/shared-react/hooks/lists";
...
export default function BookmarkOptions({ bookmark }: { bookmark: ZBookmark }) {
  ...
  const actionItems: ActionItemType[] = [
    ...
    {
      // "Delete" semantic = move to trash (archive). Recoverable from
      // /dashboard/archive. Permanent delete lives in the archive view +
      // settings, not the per-card menu, so a click can't lose data.
      // Dedup is archive-aware (re-saving the same URL after trash works).
      id: "delete",
      title: bookmark.archived ? t("actions.delete") : "Move to Trash",
      icon: <Trash2 className="mr-2 size-4" />,
      visible: isOwner,
      disabled: demoMode,
      className: bookmark.archived ? "text-destructive" : "",
      onClick: () => {
        if (bookmark.archived) {
          // Already trashed — permanent delete (existing dialog).
          setDeleteBookmarkDialogOpen(true);
        } else {
          // Move to trash — silent, no dialog (it's recoverable).
          updateBookmarkMutator.mutate({
            bookmarkId: bookmark.id,
            archived: true,
          });
