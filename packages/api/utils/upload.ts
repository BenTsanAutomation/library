...
import { assets, AssetTypes } from "@library/db/schema";
import { QuotaService, StorageQuotaError } from "@library/shared-server";
import {
  newAssetId,
  saveAssetFromFile,
...
> {
  ...
  try {
    tempFilePath = path.join(os.tmpdir(), `library-upload-${Date.now()}`);
    await pipeline(
