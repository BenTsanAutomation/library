...
import { libraryClient, mcpServer, turndownService } from "./shared";
...
machine learning is:fav`),
  ...
  async ({ query, limit, nextCursor }): Promise<CallToolResult> => {
    const res = await libraryClient.GET("/bookmarks/search", {
      params: {
        query: {
          q: query,
          limit: limit,
...
mcpServer.tool(
  ...
  async ({ bookmarkId }): Promise<CallToolResult> => {
    const res = await libraryClient.GET(`/bookmarks/{bookmarkId}`, {
      params: {
        path: {
          bookmarkId,
        },
...
mcpServer.tool(
  ...
  async ({ title, type, content }): Promise<CallToolResult> => {
    const res = await libraryClient.POST(`/bookmarks`, {
      body:
        type === "link"
          ? {
              type: "link",
...
mcpServer.tool(
  ...
  async ({ bookmarkId, ...fields }): Promise<CallToolResult> => {
    const patchRes = await libraryClient.PATCH(`/bookmarks/{bookmarkId}`, {
      params: {
        path: {
          bookmarkId,
        },
    ...
    const getRes = await libraryClient.GET(`/bookmarks/{bookmarkId}`, {
      params: {
        path: {
          bookmarkId,
        },
