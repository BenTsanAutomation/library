...
import { assets, AssetTypes, subscriptions, users } from "@library/db/schema";
...
vi.mock("@library/shared/config", async (original) => {
  const mod = (await original()) as typeof import("@library/shared/config");
  return {
    ...mod,
    default: {
      ...mod.default,
  ...
  describe("createCheckoutSession", () => {
    test<CustomTestContext>("creates checkout session for new customer", async ({
      ...
      expect(mockCheckoutSessionsCreate).toHaveBeenCalledWith({
        ...
        mode: "subscription",
        success_url:
          "https://test.library.com/settings/subscription?success=true",
        cancel_url:
          "https://test.library.com/settings/subscription?canceled=true",
        metadata: {
          userId: user.id,
        },
        customer_update: {
  ...
  describe("createPortalSession", () => {
    test<CustomTestContext>("creates portal session for user with subscription", async ({
      ...
      expect(mockBillingPortalSessionsCreate).toHaveBeenCalledWith({
        customer: "cus_123",
        return_url: "https://test.library.com/settings/subscription",
      });
    });
