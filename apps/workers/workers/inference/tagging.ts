...
import type { ZOpenAIRequest } from "@library/shared-server";
import type {
  InferenceClient,
  InferenceResponse,
} from "@library/shared/inference";
import type { ZTagStyle } from "@library/shared/types/users";
...
import { ASSET_TYPES, readAsset } from "@library/shared/assetdb";
import serverConfig from "@library/shared/config";
import logger from "@library/shared/logger";
import { buildImagePrompt } from "@library/shared/prompts";
import { buildTextPrompt } from "@library/shared/prompts.server";
import { DequeuedJob, EnqueueOptions } from "@library/shared/queueing";
import { RuleEngine } from "@library/trpc/lib/ruleEngine";
import { Bookmark } from "@library/trpc/models/bookmarks";
import { WebhooksService } from "@library/trpc/models/webhooks.service";

const openAIResponseSchema = z.object({
  tags: z.array(z.string()),
  // OpenAI's strict structured-outputs API requires every field to be either
  // required or nullable — `.optional()` alone fails schema validation. Use
  // `.nullish()` so the API accepts the schema and we still treat missing
  // contentType as "no signal".
  contentType: z
    .enum(["article", "product", "book", "recipe", "video", "note", "tweet"])
    .nullish(),
});
...
async function inferTags(
  ...
  try {
    ...
    logger.info(
      `[inference][${jobId}] Inferring tag for bookmark "${bookmark.id}" used ${response.totalTokens} tokens and inferred: ${tags} (contentType=${contentType})`,
    );
...
export async function runTagging(
  ...
  await connectTags(bookmarkId, result.tags, bookmark.userId);

  if (result.contentType) {
    await db
      .update(bookmarks)
      .set({ contentType: result.contentType })
