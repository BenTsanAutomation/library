...
function parseLibraryBookmarkFile(textContent: string): ParsedImportFile {
...
function parseInstapaperBookmarkFile(textContent: string): ParsedBookmark[] {
  ...
  return parsed.data.map((record): ParsedBookmark => {
    ...
    if (record.Folder === "Archive") {
      ...
      // Instapaper "Starred" should map to favorites in library, but
...
export function parseImportFile(
  ...
  if (source === "library") {
    const parsed = parseLibraryBookmarkFile(textContent);
    return {
