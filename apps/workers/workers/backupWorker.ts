...
import type { ZBackupRequest } from "@library/shared-server";
...
import { saveAssetFromFile } from "@library/shared/assetdb";
import {
  toExportFormat,
  toExportListFormat,
} from "@library/shared/import-export";
import logger from "@library/shared/logger";
import { DequeuedJob, getQueueClient } from "@library/shared/queueing";
import { AuthedContext } from "@library/trpc";
import { Backup } from "@library/trpc/models/backups";
import { List } from "@library/trpc/models/lists";
...
async function run(req: DequeuedJob<ZBackupRequest>) {
  ...
  const tempJsonPath = join(
    tmpdir(),
    `library-backup-${userId}-${timestamp}.json`,
  );
  const tempZipPath = join(
    tmpdir(),
    `library-backup-${userId}-${timestamp}.zip`,
  );
...
async function createZipArchiveFromFile(
  ...
  return new Promise((resolve, reject) => {
    ...
    const jsonFileName = `library-backup-${timestamp}.json`;
