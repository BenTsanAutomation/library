...
function getOpenApiDocumentation() {
  ...
  return generator.generateDocument({
    ...
    info: {
      ...
      title: "Library API",
      description:
        "Library is a self-hostable bookmarking and read-it-later service. " +
        ...
        "You can generate an API key from the Library web UI under **Settings > API Keys**.\n\n" +
        "## Pagination\n\n" +
    ...
    servers: [
      {
        ...
        variables: {
          address: {
            default: "https://try.library.example.com",
            description: "The address of the Library server",
          },
        },
      },
    ],
...
function writeDocumentation() {
  ...
  fs.writeFileSync(`./library-openapi-spec.json`, fileContent, {
    encoding: "utf-8",
  });
}
...
function checkDocumentation() {
  const docs = getOpenApiDocumentation();
  const fileContent = JSON.stringify(docs, null, 2);
  const oldContent = fs.readFileSync(`./library-openapi-spec.json`, {
    encoding: "utf-8",
  });
