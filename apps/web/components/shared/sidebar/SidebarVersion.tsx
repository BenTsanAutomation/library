...
const GITHUB_REPO_URL = GITHUB_OWNER_REPO ? `https://github.com/${GITHUB_OWNER_REPO}` : "";
const GITHUB_RELEASE_URL = GITHUB_OWNER_REPO ? `${GITHUB_REPO_URL}/releases/tag/` : "";
const RELEASE_API_URL = GITHUB_OWNER_REPO ? `https://api.github.com/repos/${GITHUB_OWNER_REPO}/releases/tags/` : "";
const LOCAL_STORAGE_KEY = "library:whats-new:last-seen-version";
...
export default function SidebarVersion({
  ...
  const versionLabel = `Library v${displayVersion}`;
  ...
  if (!stableRelease || disableNewReleaseCheck || !GITHUB_OWNER_REPO) {
    return (
      <span className="mt-auto flex items-center border-t pt-2 text-sm text-gray-400">
        {versionLabel}
      </span>
    );
  }
